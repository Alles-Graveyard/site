        ...
        <h1>Account Activation</h1>
        {
            props.reservation ? (
                <form onSubmit={e => {
                    ...
                    setFormLoading(true);
                    axios.post(`${config.apiUrl}/reservation`, {
                      password,
                    ...
                    <Button disabled={formLoading} wide>Activate</Button>
                </form>
            ) : (
...
const getReservation = async (code) => {
    var apiReq;
    try {
        apiReq = await axios.get(`${config.apiUrl}/reservation?code=${encodeURIComponent(code)}`);
    } catch (err) {
        return;
    }
